			...
			<div class="col-md-12" id="groupModalContent">
				...
				<div class="d-flex justify-content-end align-items-center mt-2">
					<div class="input-group">
						<input type="text" class="form-control" placeholder="그룹 채팅방 제목을 입력해주세요..." id="title">
						<input type="button" class="btn btn-primary" id="groupModalConfirmBtn" value="초대">
					</div>
				</div>
			</div>
		</div>
...
function createRecipientHTML(msg) {
	...
	html += '		<h6 class="fs-2 text-muted"><span name="readCount" class="text-warning fw-bold me-2" data-read-id="'+msg.emplNo+'">'+msg.readCount+'</span>'+rgsde+'</h6>';
	html += '	</div>';
	html += '</div>';
...
function createSenderHTML(msg) {
	...
	html += '	</div>';
	html += '	<div class="d-block fs-2"><span name="readCount" class="text-warning fw-bold me-2" data-read-id="'+msg.emplNo+'">'+msg.readCount+'</span>';
	html += '		'+ rgsde +'';
	html += '	</div>';
	html += '</div>';
