			...
			<div class="modal-header">
				...
				<form id="suppliesForm" class="reservation-form" style="display: none;">
					<div class="modal-body">
						...
						<div >
							<label for="erntQy">Quantity:</label>
	   						 <input type="number" id="erntQy" name="erntQy" placeholder="수량입력">
	   						 <button type="button" onclick="submitReservation()">재고체크</button>
	   						 <div id="message"></div>
						</div>
					     시작시간 :
      ...
      calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventClick: function(info) {
            ...
            // 현재 시간과 이벤트의 시작 시간 비교
              if (new Date() < eventEnd && loggedInUserNo === eventUserNo) {
                if (loggedInUserNo === eventUserNo) {  // 로그인한 사용자와 이벤트의 사용자 번호가 일치하는 경우
                    var eventData = {
                        id: info.event.id,
                ...
                } else {  // 로그인한 사용자가 아닌 경우
                    console.log("Unauthorized access attempt by user ID:", loggedInUserNo);
                }
            } else {  // 이벤트가 과거에 있을 경우
                console.log("Attempt to access past event by user ID:", loggedInUserNo);
            }
        },
...
function saveReservation(formData) {
    ...
    switch(formData.resourceType) {
        case 'room':
            ...
            // 관리자 대여 신청 request
        case 'vehicle':
            url = '/admin/rental/addVehicleReservation'; 
            break;
        case 'supplies':
            url = '/admin/rental/addSuppliesReservation';
            break;
        default:
            url = '/myReservation/addRoomReservation';
            break;
    }
    if (confirm("해당 이용을 신청하시겠습니까?")) {
	    $.ajax({
	        url: url,
	        type: 'POST',
	        processData: false,
	        ...
	        success: function(res, status, xhr) {
               if(xhr.status==200){
            	   alert('해당 이용이 신청되었습니다.');
                   location.reload(); 
               }else{
            	   showToast("예약을 등록할 수 없습니다.", "warning");
               }
	        },	
	    error: function(xhr, status, error) {
...
//     var eqpnmCd = document.getElementById('eqpnmCdIdSupplies').value; // 비품 코드
//     var erntQy = document.getElementById('erntQy').value; // 요청 수량

//     fetch(`/checkStock?eqpnmCd=\${eqpnmCd}&erntQy=\${erntQy}`, {
//         method: 'GET'
//     })
//     .then(response => response.json())
//     .then(data => {
//         document.getElementById('message').textContent = data.message; // 서버 응답 메시지 출력
//     })
//     .catch(error => {
//         console.error('Error:', error);
//         document.getElementById('message').textContent = '재고 체크에 실패했습니다.';
//     });
// }
